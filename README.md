...
- [`11.10`, `11.10.4`, `latest` (*11.10/Dockerfile*)](https://github.com/twang2218/gitlab-ce-zh/blob/master/11.10/Dockerfile)
...
```bash
docker run -d -p 3000:80 twang2218/gitlab-ce-zh:11.10.4
```

*可以将 `11.10.4` 换成你所需要的版本标签。*
...
services:
    gitlab:
      image: 'twang2218/gitlab-ce-zh:11.10.4'
...
docker run -d \
    ...
    --network gitlab-net \
    twang2218/gitlab-ce-zh:11.10.4
```
...
* `testing` 是比较 [xhang 翻译项目](https://gitlab.com/xhang/gitlab) 的 `v11.10.4` 标签和 [`11-10-stable-zh` 分支](https://gitlab.com/xhang/gitlab/tree/11-10-stable-zh) 的差异生成汉化补丁，并基于官方镜像 `gitlab/gitlab-ce:11.10.4-ce.0` 应用汉化结果进行构建的。
...
各个小版本分支的 `Dockerfile`，如 `11.10/Dockerfile` 等，都是使用 `generate` 命令生成的。因此只需要更新 `build-version.sh` 中的相关变量就可以完成各个版本镜像的升级。
...
例如：`./build.sh branch 11.10.4-ce.0 v11.10.4 8-15-stable-zh`

这表明将使用 `gitlab/gitlab-ce:11.10.4-ce.0` 做为基础镜像，并且使用上游版本标签 `v11.10.4` 作为对比的基础标签版本，也就是对应于基础镜像版本的标签，然后使用汉化分支 `8-15-stable-zh` 进行对比，生成汉化补丁，由此构建一个名为 `twang2218/gitlab-ce-zh:8-15-stable-zh` 的镜像。

### `tag` - 构建某个汉化标签的镜像

格式为：`./build.sh tag <基础镜像标签> <英文版本标签>`

例如： `./build.sh tag 11.10.4-ce.0 v11.10.4`

这表明将使用 `gitlab/gitlab-ce:11.10.4-ce.0` 镜像为基础镜像，以 `v11.10.4` 为基础对比版本，以 `v11.10.4-zh` 为汉化版本进行对比生成汉化补丁，并构建一个名为 `twang2218/gitlab-ce-zh:11.10.4` 的镜像。

### `run` - 运行某个构建好的镜像

格式为：`./build.sh <镜像标签>`

例如： `./build.sh run 11.10.4`

这将会以命令 `docker run -d -P twang2218/gitlab-ce-zh:11.10.4` 来运行镜像。这里使用的是 `-P`，因此会随机映射端口。方便测试环境测试，避免和其它端口冲突。
